...
AWS_REGION=$AWS_REGION helm repo add stable-petclinicapp s3://petclinic-helm-charts-tanerspetclinic/stable/myapp/ || echo "repository name already exists"
AWS_REGION=$AWS_REGION helm repo update
helm package k8s/petclinic_chart
AWS_REGION=$AWS_REGION helm s3 push --force petclinic_chart-${BUILD_NUMBER}.tgz stable-petclinicapp
...
AWS_REGION=$AWS_REGION helm upgrade --install \
    petclinic-app-release stable-petclinicapp/petclinic_chart --version ${BUILD_NUMBER} \
